var confetti = {
  maxCount: 96, //set max confetti count
...
(function () {
  ...
  let deadline = new Date("11/22/2020");
  deadline = new Date(deadline - 15 * 60 * 1000);

  const timer = document.getElementById("timer");
  const days = timer.querySelector("#days");
  const hours = timer.querySelector("#hours");
  const mins = timer.querySelector("#mins");
  const secs = timer.querySelector("#secs");
  setCurrentTime();
  setInterval(setCurrentTime, 1000);

  function setCurrentTime() {
    let timeInMilliSeconds =
      deadline - Date.now() >= 0 ? deadline - Date.now() : 0;

    let secval = timeInMilliSeconds / 1000;
    let mval = secval / 60;
    let hval = mval / 60;
    let dval = hval / 24;

    days.innerHTML = Math.floor(dval);
    hours.innerHTML = Math.floor(hval % 24);
    mins.innerHTML = Math.floor(mval % 60);
    secs.innerHTML = Math.floor(secval % 60);
  }
  ...
  document.addEventListener("scroll", (event) => {
    if (
      resultsPage.getBoundingClientRect().top <= 32 &&
      resultsPage.getBoundingClientRect().top >= -720
    ) {
      if (stop) startConfetti();
    } else {
      stopConfetti();
    }
  });
